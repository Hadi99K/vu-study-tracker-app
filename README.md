# VU Study Tracker

Master your semester. Track grades, assignments, and study schedules offline.

## About
Stay on top of your academic goals with **VU Study Tracker**, the dedicated app designed specifically for Virtual University students. 
Whether you're managing multiple courses, calculating your projected CGPA, or keeping track of assignment deadlines, VU Study Tracker puts everything you need right in your pocket—completely offline.

<p align="center">
  <img src="assets/images/splash.png" width="200" alt="Banner" />
</p>

## Key Features

📚 **Course Management**
Organize your semester with ease. Add your courses, track credit hours, and manage your workload efficiently.

📊 **Smart Gradebook & CGPA Calculator**
Stop guessing your grades. Input your exam results and assignment scores to instantly calculate your GPA and CGPA. Visual graphs help you track your performance trends over time.

📅 **Smart Calendar & Reminders**
Never miss a deadline again. Schedule your exams, quizzes, and study sessions with our intuitive calendar. Set custom remainders for upcoming events and build a solid study routine.

📝 **Exams Page**
Manage all your exams in one place. Track Mid Term and Final Term papers, set dates and locations, see countdown to your next paper, and export directly to Google Calendar with automatic reminders.

📂 **Offline Course Materials**
Save your handouts, past papers, and notes directly in the app. PDFs are linked to your courses, so you can study anytime, anywhere, without needing an active internet connection.

🔒 **Privacy First**
Your data belongs to you. VU Study Tracker works 100% offline and stores all your academic data locally on your device. No unwanted tracking, no external servers.

## Quick Start
1. **Add Courses**: Go to Dashboard -> Add your semester courses.
2. **Set Schedule**: Use the Calendar to plan your study sessions.
3. **Track Grades**: Enter your results in the Gradebook to see your CGPA.

## Technical Details
- **Built with**: React Native & Expo
- **Database**: SQLite (Local)
- **Status**: Beta / Pre-Launch

## Privacy
We do not collect any personal data. All information is stored locally on your device.
See our [Privacy Policy](PRIVACY.md) for details.
